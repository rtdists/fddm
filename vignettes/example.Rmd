...
<ul>
  ...
  <ul>
    ...
    <li><a href="#ddm-all">Fitting the Entire Dataset</a></li>
  </ul></li>
...
## Fitting the Entire Dataset {#ddm-all}
Here is an example of when things go wrong. I've inserted a `browser()` into the ddm file right before making the fddm fit object (line 652). 
...
med_dec <- med_dec[which(med_dec[["rt"]] >= 0), ]
nov_data1 <- med_dec[med_dec[["id"]] == "1" & med_dec[["group"]] == "novice", ]

opc <- options(contrasts = c("contr.sum", "contr.poly"))

fit1 <- ddm(rt + response ~ classification*difficulty, data = nov_data1)

# run after inside browser()
f <- new(fddm_fit, rt, response_vec, all_mm, args_ddm[["err_tol"]])
```

From here, the log-likelihood doesn't evaluate properly because one of the response times (the 123rd) is less than the EZ-diffusion calculation for `t0`. The EZ-diffusion calculation is done around line 540 in the ddm R file.

```{r ex2}
f$calculate_loglik(init_vals) # 1e6

f$rt[123] # 0.426
init_vals[6] # 0.487403
```
...
We use the same contrasts as before.

```{r set-contrasts, eval=TRUE}
opc <- options(contrasts = c("contr.sum", "contr.poly"))
```

Using the same protocol as when fitting a single individual's data, we perform the three fits. Note that the drift rate is modeled using the grand mean (labeled "Intercept") with the additional coefficients corresponding to the differences from the grand mean, according to their classification ("blast" or "non-blast") and difficulty ("easy" or "hard"). Also, the boundary separation and non-decision time are estimated by a single coefficient each, and the initial bias and inter-trial variability in the drift rate are held fixed at 0.5 and 0, respectively.

```{r multi-ddm-fit, eval=TRUE}

```

The output of the fits is structured the same as it was when only fitting a single individual's data. The raw output here may not be easily digested, so we will make a simple plot of the fitting data in the next section.
...
### Rudimentary Analysis {#ddm-all-ana}
To show any differences arising from medical experience, we will plot the drift rate parameters from each experience group.
