      ...
      for (w in 1:nW) {
        for (sv in 1:nSV) {
          ...
          RWiener = dwiener(RT, resp = resp, alpha = A[a],
                            ...
                            give_log = FALSE),
          Kesselmeier = fs(t = RT-t0, a = A[a], v = V[v],
        ...
        for (w in 1:nW) {
          for (sv in 1:nSV) {
            ...
            RWiener = dwiener(RT[rt], resp = resp, alpha = A[a],
                              delta = V[v], tau = t0, beta = W[w],
                              give_log = FALSE),
            Kesselmeier = fs(t = RT[rt]-t0, a = A[a], v = V[v],
                             w = W[w], eps = err_tol), # only "lower" resp
...
Names_vec <- c("fs_Fos_17", "fs_Fos_14",
               "fs_Kes_17", "fs_Kes_14", "fs_Nav_17", "fs_Nav_14",
               "fb_Kes_17", "fb_Kes_14", "fb_Nav_17", "fb_Nav_14",
               "fl_Nav_09", "RWiener", "Kesselmeier", "rtdists")
...
mbm_meq %>%
  ...
  facet_wrap("FuncName", scales = "free_y")
```
The relative starting point $w$ has very little impact on the infinite sum, and thus we see that the means are relatively flat across the range of $w$. Whereas $w$ does appear in the Kesselmeier method and the Foster method for approximating the infinite sum, it ultimately has very little effect on the evaluation time when compared with other parameters such as the "effective response time".
...
## Generating Benchmark Data {#fit-gen}
...
rt_fit <- function(data, id_idx = NULL, rt_idx = NULL, response_idx = NULL,
  ...
  for (i in 1:nids) {
    ...
    for (j in 1:ninit_vals) {
      ...
      temp <- nlminb(init_vals[j,], ll_fs_Fos_17, err_tol = err_tol,
      ...
      temp <- nlminb(init_vals[j,], ll_fs_Kes_17, err_tol = err_tol,
                     rt = rti, resp = respi, truth = truthi,
                     # limits:   v1,   v0,   a,      t0, w,  sv
                     lower = c(-Inf, -Inf,   0,       0, 0,   0),
                     upper = c( Inf,  Inf, Inf, min_rti, 1, Inf))
      ...
      temp <- nlminb(init_vals[j,], ll_fb_Kes_17, err_tol = err_tol,
                     rt = rti, resp = respi, truth = truthi,
                     # limits:   v1,   v0,   a,      t0, w,  sv
                     lower = c(-Inf, -Inf,   0,       0, 0,   0),
                     upper = c( Inf,  Inf, Inf, min_rti, 1, Inf))
      ...
      # microbenchmark
      mbm <- microbenchmark(
        fs_Fos_17 = nlminb(init_vals[j,], ll_fs_Fos_17, err_tol = err_tol,
                           rt = rti, resp = respi, truth = truthi,
                           # limits:   v1,   v0,   a,      t0, w,  sv
                           lower = c(-Inf, -Inf,   0,       0, 0,   0),
                           upper = c( Inf,  Inf, Inf, min_rti, 1, Inf)),
        fs_Kes_17 = nlminb(init_vals[j,], ll_fs_Kes_17, err_tol = err_tol,
                           rt = rti, resp = respi, truth = truthi,
                           # limits:   v1,   v0,   a,      t0, w,  sv
                           lower = c(-Inf, -Inf,   0,       0, 0,   0),
                           upper = c( Inf,  Inf, Inf, min_rti, 1, Inf)),
        fs_Nav_17 = nlminb(init_vals[j,], ll_fs_Nav_17, err_tol = err_tol,
                           ...
                           lower = c(-Inf, -Inf,   0,       0, 0,   0),
                           upper = c( Inf,  Inf, Inf, min_rti, 1, Inf)),
        fb_Kes_17 = nlminb(init_vals[j,], ll_fb_Kes_17, err_tol = err_tol,
                           rt = rti, resp = respi, truth = truthi,
                           # limits:   v1,   v0,   a,      t0, w,  sv
                           lower = c(-Inf, -Inf,   0,       0, 0,   0),
                           upper = c( Inf,  Inf, Inf, min_rti, 1, Inf)),
