...
The DDM is a widely used model, and a lot of effort has been put into making the evaluation of its density as fast as possible. This vignette is designed to compare the speed of the currently available algorithms to calculate the lower probability density of the DDM. To this aim, we present benchmark data of currently available methods from the literature, our streamlined implementations of those methods, and our own novel method. The [first section](#den) will show the results of benchmarking just the approximations to the density function themselves, and the [second section](#fit) will show the enchmark results of parameter estimation that uses the density approximations in its optimization.
...
Running the benchmark tests many times for each set of parameters generates a lot of benchmark data but takes a long time, so we only generate a small number of samples at each set of parameters. For the benchmark tests with 10,000 samples at each point in a more granular parameter space, see this thing that will probably be another pkgdown vignette elsewhere because the files are too big for an html_vignette.
...
This code chunk defines the function that we will use to benchmark the algorithms; we will be using the `microbenchmark` package to time the evaluations.
```{r bm-fun, eval=FALSE}
library("fddm")
library("rtdists")
library("RWiener")
...
rt_benchmark <- function(RT, resp, V, A, t0 = 1e-4, W = 0.5, SV = 0.0,
                         err_tol = 1e-6, times = 1000, unit = "us")
{
  if (resp < 0.5) {
    rresp <- "lower"
  } else {
...
bm <- rt_benchmark(RT = RT, resp = 0, V = V, A = A, t0 = t0, W = W, SV = SV,
                    err_tol = err_tol, times = 1000, unit = "us")
```
