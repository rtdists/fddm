...
fit <- rt_fit(med_dec, id_idx = c(2,1), rt_idx = 8, response_idx = 7,
              truth_idx = 5, response_upper = "blast", err_tol = 1e-8,
              times = 6, unit = "us")
```
...
fit_prep <- function(fit) {
  ...
  eps <- 1e-4
  # fit$Obj_diff[fit$Obj_diff > eps] <- eps
...
We will now present three plots to illustrate how the various methods perform when used in fitting Ratcliff's DDM to real-world data. The measures of interest for each method are the median `microbenchmark` timings, the number of optimization iterations, and the number of function evaluations in the optimization process; each one of the three plots will focus on one of these measures. Each plot will contain violin plots to show the densities of each method with respect to that plot's measure. To help explain problematic fits, the plots will indicate any data point that represents an optimization run for which the objective difference is greater than our predefined tolerance of $\epsilon = 0.0001$. More specifically, the transparency of the data points are determined by their objective difference; so a data point with zero objective difference will not show up on the plots, but a data point with objective difference close to or greater than $\epsilon$ will be visible on the plots. In addition, each of these visible data points will have a shape to indicate its convergence code: "O" for successful convergence, and "X" for failed convergence. For each method in each of the plots, the data points are aggregated over the distinct individuals in the experiment and the different initial parameter values for the fitting.
...
ggplot(fit_mbm, aes(x = factor(Algorithm, levels = Names),
       ...
       scale_alpha_continuous(range = c(0, max_alpha),
                              ...
                              labels = 
                                c(0, 
                                  formatC(max_alpha*eps/2, format = "e", digits = 2),
                                  formatC(max_alpha*eps, format = "e", digits = 2))) +
       stat_summary(fun = mean, geom = "errorbar",
                    aes(ymax = ..y.., ymin = ..y..),
