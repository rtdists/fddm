...
data <- subset(med_dec, id == 2 & group == "novice")
fit <- rt_fit(med_dec, id_idx = c(2,1), rt_idx = 8, response_idx = 7,
              truth_idx = 5, response_upper = "blast", err_tol = 1e-6)
save(fit, file = "inst/extdata/fit.Rds", compress = "xz", compression_level = 9)
```
...
fit[(0:5)*11+1,] # print results for `ID = "experienced 2"``
  ...
  for (j in 1:ninit) {
    for (k in 0:(nalgos-2)) {
      fit[(i-1)*(ninit*nalgos) + k*ninit + j, dif_idx] <-
        fit[(i-1)*(ninit*nalgos) + k*ninit + j, fit_idx] - # every method
        fit[(i-1)*(ninit*nalgos) + (nalgos-1)*ninit + j, fit_idx] #rtdists
...
2) the log-likelihood objective value differs from that produced using `rtdists`; or

3) or the parameter estimates differ from that produced using `rtdists`.
...
out <- fit2[unique(which(abs(fit2[,c(3,dif_idx)]) > eps, arr.ind = TRUE)[,1]),]
out[,-c(1:2)] <- zapsmall(out[,-c(1:2)])

out
```

We can see that it is mostly the small time methods that have occasional issues with data fitting. Although there are two method that appears in the above output that use both the small-time and large-time approximations, the method that combines the large-time approximation of Navarro with the small-time approximation of Kesselmeier is actually quite stable. Using this method, there are two offending fitted parameters whose difference from the fits given using `rtdists` are not smaller than our given error tolerance; in this case, the differences are only very marginally greater than our given error tolerance. These results suggest that the most stable methods are those that use both the small-time and large-time approximations to the density, and in particular the default option for `dfddm`, `scale = "both", n_terms_small = "Kesselmeier"`, is very stable .
