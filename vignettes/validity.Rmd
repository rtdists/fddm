...
saveRDS(fit, file = "inst/extdata/test-fitting.Rds")
```
...
fit <- readRDS(system.file("extdata", "test-fitting.Rds", package = "fddm", mustWork = TRUE))
...
start <- 1
stop <- nrow(subset(fit, id == ids[1] & Algorithm == algos[1]))
for (i in 1:nids) {
  avg_df$id[((i-1)*nalgos+1):(i*nalgos)] <- rep(ids[i], nalgos) # label id
  for (j in 1:nalgos) {
    if (j == 3) { # Navarro_s, see KE 5
      tmp_fits <- subset(fit, id == ids[i] & Algorithm == algos[j] & init_a > 0.75)[, 9:14]
    } else{
      tmp_fits <- subset(fit, id == ids[i] & Algorithm == algos[j])[, 9:14]
    }
...
Navarro_s <- subset(fit, Algorithm == "fs_Nav_17" & init_a > 0.75) # see KE 5
