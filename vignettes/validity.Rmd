...
2) Navarro large-time approximation is unstable for "small" effective response times, $\frac{rt}{a^2} \leq 0.06$. Similarly to the Navarro small-time approximation above, the large-time approximation gives slightly negative densities for effective response times in this range. Furthermore, it yields inaccurate densities for effective response times of $\frac{rt}{a^2} \leq 0.009$. These parameter values should not have an effect on the "both" time scale because the small-time approximation should handle such locations in the parameter space.
...
4) Kesselmeier_R approximation divides the error tolerance by the multiplicative term outside of the summation. Since the outside term is different when $sv > 0$, the approximation uses the incorrect error tolerance for $sv > 0$. This affects the number of terms required in the summation to achieve the desired precision, thus not actually achieving that desired precision. This issue is fixed in our implementation of the Kesselmeier method, `n_terms_small = "Kesselmeier"`, `scale = "small"`. For an example of this discrepancy, see the code below:
...
rt_fit <- function(data, id_idx = NULL, rt_idx = NULL, response_idx = NULL,
  ...
  res$v1_init <- init_vals$v1 # label initial v1
  res$v0_init <- init_vals$v0 # label initial v0
  ...
  for (j in 1:ninit) {
    for (k in 0:(nalgos-2)) {
      fit[(i-1)*(ninit*nalgos) + k*ninit + j, dif_idx] <-
        abs(fit[(i-1)*(ninit*nalgos) + (nalgos-1)*ninit + j, fit_idx] - #rtdists
            fit[(i-1)*(ninit*nalgos) + k*ninit + j, fit_idx])
    }
  }
}
```

Output below are all of the runs where either:

1) the optimization run did not converge; or

2) the log-likelihood objective value or the parameter estimates differ from that produced using `rtdists`.

Before showing which estimates were sufficiently different from the others, we first subset the fitted data to remove those fits that used the Navarro small-time approximation with an initial value of $a \leq .75$. See [Known Error 5](#fit-ke) for more details.
```{r fit-estimates, eval=TRUE}
# Account for KE 5
fit2 <- subset(fit, Algorithm != "fs_Nav_17" | a_init > .75)

# Define error tolerance
eps <- 1e-4

fit[unique(which(fit[,c(3,dif_idx)] > eps)%%nrow(fit)),]
```

We can see that only the small time methods have occasional issues with data fitting. Although there is one method that appears in the above output that uses both the small-time and large-time approximations, the difference between the offending fitted parameter $sv$ and the fit given using `rtdists` is only very marginally greater than our given error tolerance. These results suggest that the most stable methods are those that use both the small-time and large-time approximations to the density.
