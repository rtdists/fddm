...
It is imperative to show that our density functions produce the same results as the current standards. To demonstrate this, we calculate the lower probability density for a granular parameter space and show that all of the results are within an acceptable error tolerance. The parameter space can be found in the code chunk below, and it includes both realistically feasible values and some extreme values for each parameter.
...
Here is where we actually evaluate the tests (using testthat package). We have to amend some of the checks because of the instability of some of the methods, and these are documented in the section below. If all tests pass correctly, there should be no output.
...
Show results of testthat-fitting

Perhaps the most practical use of `dfddm` is to use it in an optimization setting, such as fitting DDM parameters to real world data. This section will walk through an example of using `dfddm` in this manner.

As discussed in the Known Errors section below, we restrict the starting values of the parameter $\texttt{a} \geq 0.9$ to prevent fitting issues with some of the older small-time methods.

The following code chunk runs the full fitting tests, but this takes a looooong while so we'll just read the data from file later on.
