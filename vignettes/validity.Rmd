...
This vignette is more techincal in nature and will demonstrate not only the consistency across the different implemented methods but show that `dfddm` is in accordance with other implementations in the literature. In addition to testing all of the methods available in `dfddm`, we also include three functions that are considered to be current and widely used. First we include the function `ddiffusion` from the package [`rtdists`](https://cran.r-project.org/package=rtdists) as it is well known for being not only user friendly and feature-rich but also designed specifically for handling data with regard to distributions for response time models. Second we also test the `dwiener` function from the [`RWiener`](https://cran.r-project.org/package=RWiener) package, a bundle of software that is mainly aimed at providing an `R` language interface to calculate various functions from the Wiener process. Third we include some raw `R` code that is bundled with the paper written by @gondan2014even about improving the approximation to the density function. As this code was not yet available in an `R` package, it is part of `fddm` and can be accessed after running `source` on the corresponding file as shown below.
...
This code chunk defines the function that will run the optimization and produce the fitted parameter estimates for $v_1$, $v_0$, $a$, $w$, and $sv$. As [discussed above](#fit-gen-llf), we only use a selection of methods from those available in `dfddm`. 

We also use a different set of start values for each combination of data set and method. In a real-life situation we would probably also do so using random start values to avoid local minima. Here we do this with a fixed set of starting values to ensure that the different method produce the same results for different start points. However, there are a couple of restrictions to these initial values that we must address (we would also have to do so if we picked the start values randomly). The parameter $t_0$ must be less than the response time, so we set the starting values for $t_0$ to be strictly less than the minimum response time according to each individual in the input dataframe. In addition, the optimization algorithm encounters problems for specific values of $a$ when using the log-likelihood function that is based on the Navarro small-time density function approximation, and this is exlained in [Known Errors](#fit-ke).
...
As an example data set, we use the `med_dec` data that comes with `fddm`. This data contains the accuracy condition reported in @trueblood_impact_2018 investigating medical decision making among medical professionals (pathologists) and novices (i.e., undergraduate students). The task of participants was to judge whether pictures of blood cells show cancerous cells (i.e., blast cells) or non-cancerous cells (i.e., non-blast cells). The data set contains 200 decision per participant, pictures of 100 true cancerous cells and pictures of 100 true non-cancerous cells. 

Having set up the fitting functions in the above chunks of code, we could pass the `med_dec` data to this function and run and save the object containing all model fits in an external file. However, as this takes a long time we skip the fitting in the vignette and instead read the pre-fit parameter estimates in the subsequent code chunk. 

We also print the results for the first data set (`id = 2`) and first set of start values across the different methods to see what it looks like. This shows that at least the agreement across methods for this start values is rather good.
...
save(fit, file = "test-fitting.Rds", compress = "xz")
```

```{r fitting-run-iternal, eval=FALSE, echo=FALSE}
data(med_dec, package = "fddm")
fit <- rt_fit(med_dec, id_idx = c(2,1), rt_idx = 8, response_idx = 7,
              truth_idx = 5, response_upper = "blast", err_tol = 1e-6)
save(fit, file = "inst/extdata/test-fitting.Rds", compress = "xz")
```

```{r}
# load data, will be in the variable 'fit'
load(system.file("extdata", "test-fitting.Rds", package = "fddm", mustWork = TRUE))
fit[c(1,12,23,34,45,56),]
```
