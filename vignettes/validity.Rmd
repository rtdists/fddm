                   ...
                   stvals = NULL, err_tol = 1e-6)
{
  ...
  if (is.null(stvals)) {
    ...
    stvals <- list(#    v1, v0,   a,    t0,    w, sv
                   ...
                   list( 0,  0,   5, t0_me,  0.5,  1), # vary a
                   list( 0,  0,   1, t0_lo,  0.5,  1), # vary t0
  ...
  cnames <- c("ind", "Algorithm", "AlgoCalls",
              "init_v1", "init_v0", "init_a", "init_t0", "init_w", "init_sv",
              "fit_v1", "fit_v0", "fit_a", "fit_t0", "fit_w", "fit_sv")
  res <- data.frame(matrix(ncol = length(cnames), nrow = ninds*nstvals*nalgos))
  colnames(res) <- cnames
  ...
  res[,7] <- stvals_mat[,4] # label initial t0
  res[,8] <- stvals_mat[,5] # label initial w
  res[,9] <- stvals_mat[,6] # label initial sv
  ...
  for (i in 1:ninds) {
    ...
    for (j in 1:nstvals) {
      ...
      res[(i-1)*ni+0*nstvals+j, 10:15] <- temp$par
      ...
      res[(i-1)*ni+1*nstvals+j, 10:15] <- temp$par
      ...
      res[(i-1)*ni+2*nstvals+j, 10:15] <- temp$par
      ...
      res[(i-1)*ni+3*nstvals+j, 10:15] <- temp$par
      ...
      res[(i-1)*ni+4*nstvals+j, 10:15] <- temp$par
      ...
      res[(i-1)*ni+5*nstvals+j, 10:15] <- temp$par
    }
  }
  return(res)
}
...
for (i in 1:ninds) {
  ...
  for (j in 1:nalgos) {
    if (j == 3) { # Navarro_s, see KE 5
      tmp_fits <- subset(fit, ind == inds[i] & Algorithm == algos[j] & init_a > 0.9)[, 10:15]
    } else{
      tmp_fits <- subset(fit, ind == inds[i] & Algorithm == algos[j])[, 10:15]
    }
    avg_df[(i-1)*nalgos+j,3:8] <- colMeans(tmp_fits) # collect means
    for (k in 1:(stop-1)) {
      fit[start+k, 16:21] <- abs(fit[start, 10:15] - fit[(start+k), 10:15])
    }
    start = start + stop
  }
}
...
Navarro_s <- subset(fit, Algorithm == "fs_Nav_17" & init_a > 0.9) # see KE 5
...
test_that("Consistency within each method, using different starting values", {
  expect_true(all(Foster[,16:21] < eps))
  expect_true(all(Kesselmeier_s[,16:21] < eps))
  expect_true(all(Navarro_s[,16:21] < eps))
  expect_true(all(Kesselmeier_b[,16:21] < eps))
  expect_true(all(Navarro_b[,16:21] < eps))
  expect_true(all(rtdists[,16:21] < eps))
})
...
5) For some reason, if the starting parameter value for $a \leq 0.9$ then the Navarro small-time approximation yields wildly inaccurate parameter estimates for all of the parameters.
