...
// `src/num_funcs.cpp`, but they accept a vectors as inputs and loop over them.
// This is done to add more computation time in C++ so that the `microbenchmark`
// results contain proportionally less overhead from R.
  ...
  for (int ti = 0; ti < nt; ti++) {
    for (int wi = 0; wi < nw; wi++) {
      for (int ei = 0; ei < ne; ei++) {
        u_eps = min(-1.0, log(2 * M_PI * t[ti]*t[ti] * err[ei]*err[ei])); // Safe bound for sqrt
...
{ // note: w is not used
  ...
  for (int ti = 0; ti < nt; ti++) {
    for (int wi = 0; wi < nw; wi++) {
      for (int ei = 0; ei < ne; ei++) {
        if (err[ei] < 1 / (2 * sqrt(2 * M_PI * t[ti]))) { // if error threshold is set low enough
          ks = 2 + sqrt(-2 * t[ti] * log(2 * err[ei] * sqrt(2 * M_PI * t[ti])));
