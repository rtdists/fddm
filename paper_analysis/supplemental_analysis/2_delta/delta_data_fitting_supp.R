...
# This file produces the supplementary results (and plots) that pertain to data
# fitting using the implementations that combine the SWSE small-time method with
# the Navarro et al. large-time method, as shown in
...
# Figure S2
fig_s2 <- ggplot(fit_fev, aes(x = factor(Algorithm, levels = Names),
                              y = FuncEvals)) +
  ...
  stat_summary(aes(y = Obj_diff, color = factor(Algorithm, levels = Names)),
               ...
               fun.args = list(fit, "FuncEvals", 1.15),
               geom = "label",
               hjust = 0.5,
               vjust = 0.9) +
  ...
  labs(x = bquote(delta ~ ", and Summation Style"),
...
ggsave(paste0(img_dir, "delta_fit_fe.png"),
       plot = fig_s2, width = 16, height = 9)
