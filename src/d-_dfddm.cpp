...
#include "d-_dfddm_helper_functions/func_defs.h"
#include "d-_dfddm_helper_functions/helper_funcs.h"
#include "d-_dfddm_helper_functions/partial_funcs.h"
#include "d-_dfddm_helper_functions/sum_funcs.h"
#include "d-_dfddm_helper_functions/num_funcs.h"
...
//' @export
// [[Rcpp::export]]
NumericVector dt_dfddm(const NumericVector& rt,
                       const SEXP& response,
...
//' @export
// [[Rcpp::export]]
NumericVector dt0_dfddm(const NumericVector& rt,
                        const SEXP& response,
                        const NumericVector& v,
                        const NumericVector& a,
                        const NumericVector& t0,
...
//' @export
// [[Rcpp::export]]
NumericVector dv_dfddm(const NumericVector& rt,
                       const SEXP& response,
                       const NumericVector& v,
                       const NumericVector& a,
                       const NumericVector& t0,
...
//' @export
// [[Rcpp::export]]
NumericVector da_dfddm(const NumericVector& rt,
                       const SEXP& response,
                       const NumericVector& v,
                       const NumericVector& a,
                       const NumericVector& t0,
...
//' @export
// [[Rcpp::export]]
NumericVector dw_dfddm(const NumericVector& rt,
                       const SEXP& response,
                       const NumericVector& v,
                       const NumericVector& a,
                       const NumericVector& t0,
...
//' @export
// [[Rcpp::export]]
NumericVector dsv_dfddm(const NumericVector& rt,
                        const SEXP& response,
                        const NumericVector& v,
                        const NumericVector& a,
                        const NumericVector& t0,
...
// //' Partial Derivative of 5-parameter DDM log(PDF) with respect to t (response
// //'   time)
// //'
// //' Partial Derivative of the log density function for the 5-parameter variant
// //'   of the Ratcliff diffusion decision model (DDM) with respect to t, the
// //'   response time. This variant contains the following parameters:
...
// //'
// //'
// //' @return A vector containing the partial derivatives of the DDM log(PDF) with
// //'   respect to t (response time) that has precision \code{err_tol} whose
// //'   length matches that of the longest input parameter (usually \code{rt}).
// //'
// //' @useDynLib fddm, .registration = TRUE
// //' @import Rcpp
// //' @export
// // [[Rcpp::export]]
// NumericVector dt_log_dfddm(const NumericVector& rt,
//                            const SEXP& response,
//                            const NumericVector& a,
//                            const NumericVector& v,
//                            const NumericVector& t0,
...
//                            const NumericVector& err_tol = 0.000001)
// {
//   ParFunc parf = &pdf_dt_log;
//   return Rcpp::wrap(partial_pdf(parf, rt, response, a, v, t0, w, sv, sigma,
//                                 sl_thresh, err_tol));
// }



// //' Partial Derivative of 5-parameter DDM log(PDF) with respect to t0
// //'   (non-decision time)
// //'
// //' Partial Derivative of the log density function for the 5-parameter variant
// //'   of the Ratcliff diffusion decision model (DDM) with respect to t0, the
// //'   non-decision time. This variant contains the following parameters:
// //'   \code{a} (threshold separation), \code{v} (drift rate),
// //'   \code{t0} (non-decision time/response time constant), \code{w}
...
// //'
// //' @return A vector containing the partial derivatives of the DDM log(PDF) with
// //'   respect to t0 (non-decision time) that has precision \code{err_tol} whose
// //'   length matches that of the longest input parameter (usually \code{rt}).
// //'
// //' @useDynLib fddm, .registration = TRUE
// //' @import Rcpp
// //' @export
// // [[Rcpp::export]]
// NumericVector dt0_log_dfddm(const NumericVector& rt,
//                             const SEXP& response,
//                             const NumericVector& a,
//                             const NumericVector& v,
//                             const NumericVector& t0,
...
// //' Partial Derivative of 5-parameter DDM log(PDF) with respect to a (threshold
// //'   separation)
// //'
// //' Partial Derivative of the log density function for the 5-parameter variant
// //'   of the Ratcliff diffusion decision model (DDM) with respect to a, the
// //'   threshold separation. This variant contains the following parameters:
// //'   \code{a} (threshold separation), \code{v} (drift rate),
// //'   \code{t0} (non-decision time/response time constant), \code{w}
// //'   (relative starting point), \code{sv} (inter-trial variability of drift), and
...
// //'
// //' @return A vector containing the partial derivatives of the DDM log(PDF) with
// //'   respect to a (threshold separation) that has precision \code{err_tol}
// //'   whose length matches that of the longest input parameter (usually
// //'   \code{rt}).
// //'
// //' @useDynLib fddm, .registration = TRUE
// //' @import Rcpp
// //' @export
// // [[Rcpp::export]]
// NumericVector da_log_dfddm(const NumericVector& rt,
//                            const SEXP& response,
//                            const NumericVector& a,
//                            const NumericVector& v,
//                            const NumericVector& t0,
...
// //' Partial Derivative of 5-parameter DDM log(PDF) with respect to v (drift
// //'   rate)
// //'
// //' Partial Derivative of the log density function for the 5-parameter variant
// //'   of the Ratcliff diffusion decision model (DDM) with respect to v, the
// //'   drift rate. This variant contains the following parameters:
// //'   \code{a} (threshold separation), \code{v} (drift rate),
// //'   \code{t0} (non-decision time/response time constant), \code{w}
// //'   (relative starting point), \code{sv} (inter-trial variability of drift), and
// //'   \code{sigma} (diffusion coefficient of underlying Wiener process).
...
// //'
// //' @return A vector containing the partial derivatives of the DDM log(PDF) with
// //'   respect to v (drift rate) that has precision \code{err_tol} whose length
// //'   matches that of the longest input parameter (usually \code{rt}).
// //'
// //' @useDynLib fddm, .registration = TRUE
// //' @import Rcpp
// //' @export
// // [[Rcpp::export]]
// NumericVector dv_log_dfddm(const NumericVector& rt,
//                            const SEXP& response,
//                            const NumericVector& a,
//                            const NumericVector& v,
//                            const NumericVector& t0,
...
// //' Partial Derivative of 5-parameter DDM log(PDF) with respect to w (initial
// //'   bias)
// //'
// //' Partial Derivative of the log density function for the 5-parameter variant
// //'   of the Ratcliff diffusion decision model (DDM) with respect to w, the
// //'   inital bias. This variant contains the following parameters:
// //'   \code{a} (threshold separation), \code{v} (drift rate),
// //'   \code{t0} (non-decision time/response time constant), \code{w}
// //'   (relative starting point), \code{sv} (inter-trial variability of drift), and
// //'   \code{sigma} (diffusion coefficient of underlying Wiener process).
...
// //'
// //' @return A vector containing the partial derivatives of the DDM log(PDF) with
// //'   respect to w (initial bias) that has precision \code{err_tol} whose length
// //'   matches that of the longest input parameter (usually \code{rt}).
// //'
// //' @useDynLib fddm, .registration = TRUE
// //' @import Rcpp
// //' @export
// // [[Rcpp::export]]
// NumericVector dw_log_dfddm(const NumericVector& rt,
//                            const SEXP& response,
//                            const NumericVector& a,
//                            const NumericVector& v,
//                            const NumericVector& t0,
...
// //' Partial Derivative of 5-parameter DDM log(PDF) with respect to sv
...
// //'
// //' @return A vector containing the partial derivatives of the DDM log(PDF) with
// //'   respect to sv (inter-trial variability in the drift rate) that has
// //'   precision \code{err_tol} whose length matches that of the longest input
// //'   parameter (usually \code{rt}).
// //'
// //' @useDynLib fddm, .registration = TRUE
...
// //' @export
// // [[Rcpp::export]]
// NumericVector dsv_log_dfddm(const NumericVector& rt,
//                              const SEXP& response,
//                              const NumericVector& a,
//                              const NumericVector& v,
//                              const NumericVector& t0,
